...
trait BaseJson4sSupport extends JsonSupportInterface {
  ...
  def fromEntityToJValue(a: Any): JValue =
    Extraction.decompose(a)(json4sFormats)

  def fromEntityToString[A <: AnyRef](a: A): String =
    Serialization.write(a)(json4sFormats)
  ...
  def fromJsonToEntity[A](json: JValue)(implicit mf: Manifest[A]): A =
    json.extract[A](json4sFormats, mf)

  def fromJsonToString(json: JValue) =
    pretty(render(json)(json4sFormats))
