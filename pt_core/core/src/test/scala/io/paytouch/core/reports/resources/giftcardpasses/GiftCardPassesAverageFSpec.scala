...
class GiftCardPassesAverageFSpec extends GiftCardPassesFSpec {

  def action = "average"

  import fixtures._

  class GiftCardPassesAverageFSpecContext extends GiftCardPassesFSpecContext

  "GET /v1/reports/gift_card_passes.average" in {
    ...
    "with no interval" should {
      "with no group by" should {
        ...
        assertFieldResult(
          "customers",
          ReportFields(key = None, emptyAggregate.copy(customers = fullAggregate.customers)),
        )
    ...
    "with interval" should {
      "with no group by" should {
        ...
        "with no fields it should reject the request" in new GiftCardPassesAverageFSpecContext {
          Get(s"/v1/reports/gift_card_passes.average?$defaultParamsWithInterval")
            .addHeader(authorizationHeader) ~> routes ~> check {
            rejection ==== MissingQueryParamRejection("field[]")
          }
        }

        "with all fields" in new GiftCardPassesAverageFSpecContext {
          Get(s"/v1/reports/gift_card_passes.average?$defaultParamsWithInterval&field[]=$allFieldsParams")
            .addHeader(authorizationHeader) ~> routes ~> check {
            assertStatusOK()
      ...
      "with group by value" should {
        "with all the fields" in new GiftCardPassesAverageFSpecContext {
          Get(
            s"/v1/reports/gift_card_passes.average?$defaultParamsWithInterval&field[]=$allFieldsParams&group_by=value",
          ).addHeader(authorizationHeader) ~> routes ~> check {
            assertStatusOK()
      ...
      "with location_id" should {
        "return correct data" in new GiftCardPassesAverageFSpecContext {
          Get(
            s"/v1/reports/gift_card_passes.average?$defaultParamsWithInterval&field[]=$allFieldsParams&group_by=value&location_id=${willow1.id}",
          ).addHeader(authorizationHeader) ~> routes ~> check {
            assertStatusOK()
        ...
        "with a non-accessible location" should {
          "reject the request" in new GiftCardPassesAverageFSpecContext {
            val newYork = Factory.location(merchant).create

            Get(
              s"/v1/reports/gift_card_passes.average?$defaultParamsWithInterval&field[]=$allFieldsParams&location_id=${newYork.id}",
            ).addHeader(authorizationHeader) ~> routes ~> check {
