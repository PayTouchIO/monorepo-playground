...
class OrdersValidatedSyncFSpec extends OrdersSyncAssertsFSpec {

  "POST /v1/orders.validated_sync?order_id=$" in {
    "if request has valid token" in {
      "if the order doesn't exist" should {
        "creates the order and all its relations" in new OrdersSyncFSpecContext with Fixtures {
          ...
          val upsertion = baseOrderUpsertion.copy(
            ...
            seating = Some(random[Seating]),
          )

          Post(s"/v1/orders.validated_sync?order_id=$orderId", upsertion)
        ...
        "if coming from storefront" should {
          "create a customer id and associate it with order" in new OrdersSyncFSpecContext with Fixtures {
            val orderId = UUID.randomUUID
            val deliveryAddressUpsertion = random[DeliveryAddressUpsertion].copy(id = UUID.randomUUID)
            val customerEmail = randomEmail
          ...
          "allow acceptance status = open" in new OrdersSyncFSpecContext with Fixtures {
            val orderId = UUID.randomUUID
            val deliveryAddressUpsertion = random[DeliveryAddressUpsertion].copy(id = UUID.randomUUID)
            val onlineOrderAttributeUpsertion = randomOnlineOrderAttributeUpsertion().copy(
          ...
          "allow acceptance status = pending" in new OrdersSyncFSpecContext with Fixtures {
            val orderId = UUID.randomUUID
            val deliveryAddressUpsertion = random[DeliveryAddressUpsertion].copy(id = UUID.randomUUID)
            val onlineOrderAttributeUpsertion = randomOnlineOrderAttributeUpsertion().copy(
              email = Some(randomEmail),
          ...
          "reject acceptance status = accepted" in new OrdersSyncFSpecContext with Fixtures {
            val orderId = UUID.randomUUID
            val deliveryAddressUpsertion = random[DeliveryAddressUpsertion].copy(id = UUID.randomUUID)
            val onlineOrderAttributeUpsertion = randomOnlineOrderAttributeUpsertion().copy(
              email = Some(randomEmail),
              ...
              lastName = Some(randomWord),
              phoneNumber = Some(randomWord),
              acceptanceStatus = Some(AcceptanceStatus.Accepted),
            )
            ...
            Post(s"/v1/orders.validated_sync?order_id=$orderId", upsertion)
              ...
              assertErrorCodesAtLeastOnce("InvalidAcceptanceStatus")
            }
          }

          "reject acceptance status = rejected" in new OrdersSyncFSpecContext with Fixtures {
            val orderId = UUID.randomUUID
            val deliveryAddressUpsertion = random[DeliveryAddressUpsertion].copy(id = UUID.randomUUID)
            val onlineOrderAttributeUpsertion = randomOnlineOrderAttributeUpsertion().copy(
              email = Some(randomEmail),
        ...
        "rejects zero discounts" in new OrdersSyncFSpecContext with Fixtures {
          val anotherUser = Factory.user(merchant, locations = Seq(rome)).create
          val orderId = UUID.randomUUID
          ...
          Post(s"/v1/orders.validated_sync?order_id=$orderId", upsertion)
            .addHeader(authorizationHeader) ~> routes ~> check {
            assertStatus(StatusCodes.BadRequest)
            assertErrorCodesAtLeastOnce("ZeroDiscount")
          }
        }

        "stocks" should {
          "creates the order and track stock updates if trackInventory = true" in new OrdersSyncFSpecContext
            with Fixtures {
          ...
          "rejects orders with out of stock products if trackInventory = true, sellOutOfStock = false" in new OrdersSyncFSpecContext
            with Fixtures {
            override lazy val product = Factory.simpleProduct(merchant, trackInventory = Some(true)).create
            val productLocation = Factory.productLocation(product, london).create
            val originalStockQuantity = 2
            val stock = Factory
              .stock(productLocation, quantity = Some(originalStockQuantity), sellOutOfStock = Some(false))
              .create
          ...
          "creates orders with out of stock products if trackInventory = true, sellOutOfStock = true" in new OrdersSyncFSpecContext
            with Fixtures {
            override lazy val product = Factory.simpleProduct(merchant, trackInventory = Some(true)).create
            val productLocation = Factory.productLocation(product, london).create
            val originalStockQuantity = 2
            val stock =
              Factory.stock(productLocation, quantity = Some(originalStockQuantity), sellOutOfStock = Some(true)).create
          ...
          "creates the order and do not track stock updates if trackInventory = false" in new OrdersSyncFSpecContext
            with Fixtures {
      ...
      "if a product in an order item does not belong to the merchant" should {
        "it should reject the order" in new OrdersSyncFSpecContext with Fixtures {
          val orderId = UUID.randomUUID
          val upsertion = baseOrderUpsertion.copy(
          ...
          Post(s"/v1/orders.validated_sync?order_id=${orderId}", upsertion)
            ...
            assertErrorCodesAtLeastOnce("InvalidProductIds")
          }
        }
      }
      ...
      "if the order does not belong to the merchant" should {
        "it should reject the order" in new OrdersSyncFSpecContext with Fixtures {
          ...
          Post(s"/v1/orders.validated_sync?order_id=${competitorOrder.id}", upsertion)
            .addHeader(authorizationHeader) ~> routes ~> check {
            assertStatus(StatusCodes.NotFound)
            assertErrorCodesAtLeastOnce("InvalidOrderIds")
          }
        }
      }

      "if all the ids are random" should {
        "it should reject the order" in new OrdersSyncFSpecContext with Fixtures {
          ...
          Post(s"/v1/orders.validated_sync?order_id=$randomId", upsertion)
            .addHeader(authorizationHeader) ~> routes ~> check {
            assertStatus(StatusCodes.NotFound)
            assertErrorCodesAtLeastOnce(
              "InvalidProductIds",
              "InvalidDiscountIds",
    ...
    "if request has valid app token" should {
      "sync the order" in new OrdersSyncFSpecContext with Fixtures with AppTokenFixtures {
        ...
        Post(s"/v1/orders.validated_sync?order_id=$orderId", baseOrderUpsertion)
