...
class CustomersAverageFSpec extends CustomersFSpec {

  def action = "average"

  import fixtures._
  class CustomersAverageFSpecContext extends CustomersFSpecContext

  "GET /v1/reports/customers.average" in {
    ...
    "with no interval" should {
      "with no group by" should {
        ...
        "with field spend" in new CustomersAverageFSpecContext {
          Get(s"/v1/reports/customers.average?$defaultParamsNoInterval&field[]=spend")
            ...
            val expectedResult = buildExpectedResultWhenNoInterval(
              ReportFields(key = None, CustomerAggregate(count = 3, spend = Some(344.33.$$$))),
            )
            result ==== expectedResult
          }
        }

        "with all the fields" in new CustomersAverageFSpecContext {
          Get(s"/v1/reports/customers.average?$defaultParamsNoInterval&field[]=$allFieldsParams")
            .addHeader(authorizationHeader) ~> routes ~> check {
            assertStatusOK()
      ...
      "with group by customer type" should {
        "with all the fields" in new CustomersAverageFSpecContext {
          Get(s"/v1/reports/customers.average?$defaultParamsNoInterval&field[]=$allFieldsParams&group_by=type")
            .addHeader(authorizationHeader) ~> routes ~> check {
            assertStatusOK()
            ...
            val expectedResult = buildExpectedResultWhenNoInterval(
              ReportFields(key = CustomerType.New, CustomerAggregate(count = 2, spend = Some(508.50.$$$))),
      ...
      "with group by visit" should {
        "with all the fields" in new CustomersAverageFSpecContext {
          Get(s"/v1/reports/customers.average?$defaultParamsNoInterval&field[]=$allFieldsParams&group_by=visit")
            .addHeader(authorizationHeader) ~> routes ~> check {
            assertStatusOK()
      ...
      "with group by loyalty program" should {
        "with all the fields" in new CustomersAverageFSpecContext {
          Get(
            s"/v1/reports/customers.average?$defaultParamsNoInterval&field[]=$allFieldsParams&group_by=loyalty_program",
          ).addHeader(authorizationHeader) ~> routes ~> check {
            assertStatusOK()
            ...
            val expectedResult = buildExpectedResultWhenNoInterval(
              ...
              ReportFields(key = LoyaltyProgramStatus.Without, CustomerAggregate(count = 2, spend = Some(508.50.$$$))),
            )
            result ==== expectedResult
          }
        }
    ...
    "with interval" should {
      "with no group by" should {
        ...
        "with no fields it should reject the request" in new CustomersAverageFSpecContext {
          Get(s"/v1/reports/customers.average?$defaultParamsWithInterval")
            .addHeader(authorizationHeader) ~> routes ~> check {
        ...
        "with field spend" in new CustomersAverageFSpecContext {
          Get(s"/v1/reports/customers.average?$defaultParamsWithInterval&field[]=spend")
            .addHeader(authorizationHeader) ~> routes ~> check {
            assertStatusOK()

            val result = responseAs[ReportResponse[ReportFields[CustomerAggregate]]]
            val expectedResult = buildExpectedResultWithInterval(
              ReportData(ReportTimeframe(start, end), List(ReportFields(None, CustomerAggregate(3, Some(340.33.$$$))))),
              ReportData(
        ...
        "with all the fields" in new CustomersAverageFSpecContext {
          Get(s"/v1/reports/customers.average?$defaultParamsWithInterval&field[]=$allFieldsParams")
            .addHeader(authorizationHeader) ~> routes ~> check {
            assertStatusOK()
      ...
      "with group by customer type" should {
        "with all the fields" in new CustomersAverageFSpecContext {
          Get(s"/v1/reports/customers.average?$defaultParamsWithInterval&field[]=$allFieldsParams&group_by=type")
            .addHeader(authorizationHeader) ~> routes ~> check {
            assertStatusOK()
            ...
            val expectedResult = buildExpectedResultWithInterval(
              ReportData(
                ...
                List(
                  ReportFields(CustomerType.New, CustomerAggregate(2, Some(505.50.$$$))),
      ...
      "with group by visit" should {
        "with all the fields" in new CustomersAverageFSpecContext {
          Get(s"/v1/reports/customers.average?$defaultParamsWithInterval&field[]=$allFieldsParams&group_by=visit")
            .addHeader(authorizationHeader) ~> routes ~> check {
            assertStatusOK()
            ...
            val expectedResult = buildExpectedResultWithInterval(
              ReportData(
                ReportTimeframe(start, end),
                List(ReportFields(Some("1"), CustomerAggregate(3, Some(340.33.$$$)))),
              ),
              ReportData(
                ReportTimeframe(start.plusDays(7), end.plusDays(7)),
      ...
      "with group by loyalty program" should {
        "with all the fields" in new CustomersAverageFSpecContext {
          Get(
            s"/v1/reports/customers.average?$defaultParamsWithInterval&field[]=$allFieldsParams&group_by=loyalty_program",
          ).addHeader(authorizationHeader) ~> routes ~> check {
            assertStatusOK()
            ...
            val expectedResult = buildExpectedResultWithInterval(
              ReportData(
                ...
                List(
                  ...
                  ReportFields(LoyaltyProgramStatus.Without, CustomerAggregate(2, Some(505.50.$$$))),
                ),
              ),
              ReportData(
                ReportTimeframe(start.plusDays(7), end.plusDays(7)),
    ...
    "with location_id" should {
      "return data filtered by location id" in new CustomersAverageFSpecContext {
        Get(s"/v1/reports/customers.average?$defaultParamsNoInterval&field[]=spend&location_id=${london.id}")
          .addHeader(authorizationHeader) ~> routes ~> check {
          assertStatusOK()
