...
abstract class CategoriesFSpec extends FSpec {

  abstract class CategoryResourceFSpecContext
    ...
    def assertSubcategoryUpsertion(parentId: UUID, upsertion: SubcategoryUpsertion) = {
      ...
      if (upsertion.position.isDefined) upsertion.position.getOrElse(0) ==== model.position
