...
trait BaseHttpLogging extends StrictEntitiesDirectories with StrictEntitiesHelper { self: BaseJsonSupport =>
  ...
  private def requestBodyShouldBeLogged(loggedRequest: LoggedRequest) =
    isPostOfJson(loggedRequest) || isPostOfUrlEncodedForm(loggedRequest)
  ...
  private def isPostOfJson(request: LoggedRequest): Boolean =
    request.method == HttpMethods.POST && request.contentType == ContentTypes.`application/json`

  private def isPostOfUrlEncodedForm(request: LoggedRequest): Boolean =
    request.method == HttpMethods.POST && request
      .contentType
