...
abstract class BaseMdcActor extends Actor with DiagnosticActorLogging {
  ...
  def receive = {
    ...
    case parsedEntity: LoggedParsedEntity => log.info(parsedEntity.entity.body)
  ...
  override def mdc(currentMessage: Any): MDC =
    currentMessage match {
      ...
      case parsedEntity: LoggedParsedEntity =>
        typeMap(parsedEntity.`type`) ++ requestMap(parsedEntity.loggedRequest) ++ parsedEntity
          .loggedResponse
  ...
  private def toPrettyHeader(h: HttpHeader): String =
    if (isRedactedHeader(h))
      s"${h.name}: **REDACTED**"
    else
      s"${h.name}: ${h.value}"

  private def isRedactedHeader(h: HttpHeader) =
    h.is("authorization") || h.is("timeout-access")
}
