...
import cats.{ Apply, Semigroup, SemigroupK, Traverse }
...
trait ValidatedUtils[EE] extends LazyLogging {
  ...
  def failure[T](errors: Seq[EE]): Validation[T] = {
    logger.debug("{}", errors)
    require(errors.nonEmpty, "You must not call ValidatedUtils.failure with an empty seq of errors")
    NonEmptyList.fromList[EE](errors.toList) match {
      case Some(nel) => Invalid(nel)
      case _         => throw new RuntimeException("Called ValidatedUtils.failure() with an empty seq of errors")
    }
  }

  def success[T](t: T): Validation[T] = Valid(t)

  def sequence[T](vds: Iterable[Validation[Seq[T]]]): Validation[Seq[T]] =
    genericSequence(vds)(_ ++ _, Seq.empty)

  private def genericSequence[T](vds: Iterable[Validation[T]])(f: (T, T) => T, default: T): Validation[T] = {

    @tailrec
    def loop(remaining: Iterable[Validation[T]], soFar: Validation[T]): Validation[T] =
      remaining match {
        case Nil => soFar
        case head :: tail =>
    ...
    loop(vds, Valid(default))
  }

  def futSequence[T](
      vds: Iterable[Future[Validation[Seq[T]]]],
    )(implicit
      ec: ExecutionContext,
    ): Future[Validation[Seq[T]]] =
    Future.sequence(vds).map(sequence)

  def combineSeq[A](validations: Seq[Validation[A]]): Validation[Seq[A]] =
